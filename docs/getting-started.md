...
If BMAD install/update prompts interactively, times out in unattended mode, or returns success without producing required output files, planning falls back to generated `_bmad-output` artifacts with explicit markers so the run can continue.
